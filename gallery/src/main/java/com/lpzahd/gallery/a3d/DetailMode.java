...
public final class DetailMode {
    ...
    public static CharSequence[] populateDetailModeStrings(MediaWaiter presenter, ArrayList<MediaBucket> buckets) {
        ...
        if (MediaBucketList.isSetSelection(buckets) && numBuckets == 1) {
            ...
            return populateSetViewDetailModeStrings(presenter, MediaBucketList.getFirstSetSelection(buckets), 1);
            ...
            return populateSetViewDetailModeStrings(presenter, selectedItemsSet, numBuckets);
        } else {
            return populateItemViewDetailModeStrings(presenter, MediaBucketList.getFirstItemSelection(buckets));
        }
    }
    ...
    private static CharSequence[] populateSetViewDetailModeStrings(MediaWaiter presenter, MediaSet selectedItemsSet, int numOriginalSets) {
        ...
        Resources resources = presenter.getActivity().getResources();
        ...
        if (selectedItemsSet.mLatLongDetermined) {
            ...
            if (locationString == null) {
                ...
                ReverseGeocoder reverseGeocoder = presenter.getReverseGeocoder();
    ...
    private static CharSequence[] populateItemViewDetailModeStrings(MediaWaiter presenter, MediaItem item) {
        ...
        if (locationString == null || locationString.length() == 0) {
            locationString = presenter.getActivity().getResources().getString(R.string.location_unknown);
        }
