...
public final class GridLayer extends RootLayer implements MediaFeed.Listener, TimeBar.Listener {
    ...
    public GridLayer(MediaPresenter presenter, int itemWidth, int itemHeight, LayoutInterface layoutInterface, RenderView view) {
        ...
        sVisibleItems = new ArrayList<>();
        mHud = new HudLayer(mPresenter);
        mHud.setContext(mPresenter);
        mHud.setGridLayer(this);
        mHud.getPathBar().clear();
        mHud.setGridLayer(this);
        mHud.getTimeBar().setListener(this);
        mHud.getPathBar().pushLabel(R.drawable.icon_home_small, mPresenter.getActivity().getResources().getString(R.string.app_name),
                new Runnable() {
                    public void run() {
        ...
        mDrawManager = new GridDrawManager(mPresenter.getActivity(), mCamera, mDrawables, sDisplayList, sDisplayItems, sDisplaySlots);
        mInputProcessor = new GridInputProcessor(mPresenter, mCamera, this, mView, sTempVec, sDisplayItems);
        setState(STATE_MEDIA_SETS);
    }
    ...
    public void setDataSource(DataSource dataSource) {
        ...
        mMediaFeed = new MediaFeed(mPresenter, dataSource, this);
        mMediaFeed.start();
    }
    ...
    public void renderOpaque(RenderView view, GL11 gl) {
        ...
        if (mSelectedAlpha != 0.0f) {
            mDrawManager.drawThumbnails(mPresenter, view, gl, mState);
        }
        if (mSelectedAlpha != 1.0f) {
        ...
        if (selectedSlotIndex != Shared.INVALID) {
            mDrawManager.drawFocusItems(mPresenter, view, gl, mZoomValue, mSlideshowMode, mTimeElapsedSinceView);
    ...
    public void renderBlended(RenderView view, GL11 gl) {
        ...
        if (mHud != null && mDrawManager != null) {
            if (mMediaFeed != null) {
                mDrawManager.drawBlendedComponents(mPresenter, view, gl, mSelectedAlpha, mState, mHud.getMode(),
    ...
    public void setPickIntent(boolean b) {
        mPickIntent = b;
        mHud.getPathBar().popLabel();
        mHud.getPathBar().pushLabel(R.drawable.icon_location_small, mPresenter.getActivity().getResources().getString(R.string.pick),
                new Runnable() {
                    public void run() {
                        if (mHud.getAlpha() == 1.0f) {
                            if (!mFeedAboutToChange) {
