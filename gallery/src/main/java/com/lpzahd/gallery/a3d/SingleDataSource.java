    ...
    public void loadItemsForSet(MediaFeed feed, MediaSet parentSet, int rangeStart, int rangeEnd) {
        ...
        if (mSingleUri && !mDone) {
            ...
            if (mUri.startsWith(MediaStore.Images.Media.EXTERNAL_CONTENT_URI.toString())) {
                MediaItem newItem = LocalDataSource.createMediaItemFromUri(mPresenter.getActivity(), Uri.parse(mUri));
                if (newItem != null) {
                    item = newItem;
                ...
                int numRetries = 15;
                do {
                    newItem = LocalDataSource.createMediaItemFromFileUri(mPresenter.getActivity(), mUri);
                    if (newItem == null) {
                        --numRetries;
                        try {
                            Thread.sleep(500);
            ...
            final ContentResolver cr = mPresenter.getActivity().getContentResolver();
            String where = null;
            try {
    ...
    public void loadMediaSets(MediaFeed feed) {
        ...
        if (!mAllItems && set != null && loadOtherSets) {
            if (!CacheService.isPresentInCache(set.mId)) {
                CacheService.markDirty(mPresenter);
            }
