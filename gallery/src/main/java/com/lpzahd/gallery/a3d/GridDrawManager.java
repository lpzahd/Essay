...
public final class GridDrawManager {
        ...
        for (int itrSlotIndex = firstBufferedVisibleSlot; itrSlotIndex <= lastBufferedVisibleSlot; ++itrSlotIndex) {
            ...
            for (int j = GridLayer.MAX_DISPLAYED_ITEMS_PER_SLOT - 1; j >= 0; --j) {
                ...
                if (displayItem == null) {
                    ...
                    Texture texture = displayItem.getThumbnailImage(presenter, sThumbnailConfig);
    ...
    public void drawFocusItems(MediaPresenter presenter, RenderView view, GL11 gl, float zoomValue, boolean slideshowMode, float timeElapsedSinceView) {
        ...
        if (selectedSlotIndex != Shared.INVALID) {
            ...
            Texture centerTexture = centerDisplayItem.getScreennailImage(presenter);
            ...
            for (int i = -1; i <= 1; ++i) {
                ...
                if (selectedSlotToUse >= 0 && selectedSlotToUse <= lastBufferedVisibleSlot) {
                    ...
                    final Texture thumbnailTexture = displayItem.getThumbnailImage(presenter, sThumbnailConfig);
                    Texture texture = displayItem.getScreennailImage(presenter);
