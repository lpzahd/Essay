...
public final class MediaFeed implements Runnable {
    ...
    public ArrayList<MediaClustering.Cluster> getClustersForSet(final MediaSet set) {
        ArrayList<MediaClustering.Cluster> clusters = null;
        ...
        if (mInClusteringMode == false) {
            ...
            if (clustering != null) {
                ...
                if (clusters.size() > slotIndex) {
                    ...
                    cluster.generateCaption(mPresenter.getActivity());
                    return cluster;
                }
            }
        }
    ...
    public void start() {
        ...
        mAlbumSourceThread = new Thread(new Runnable() {
            public void run() {
                ...
                while (ImageManager.isMediaScannerScanning(mPresenter.getActivity().getContentResolver())) {
                    ...
                    try {
                        if (mPresenter == null)
                            return;
                        showToast(mPresenter.getActivity().getResources().getString(R.string.initializing), Toast.LENGTH_LONG);
                        Thread.sleep(6000);
                ...
                if (mWaitingForMediaScanner) {
                    showToast(mPresenter.getActivity().getResources().getString(R.string.loading_new), Toast.LENGTH_LONG);
                    mWaitingForMediaScanner = false;
                    if (dataSource != null) {
                        dataSource.loadMediaSets(feed);
                    }
    ...
    private void showToast(final String string, final int duration, final boolean centered) {
        if (mPresenter != null && !mPresenter.isPaused()) {
            mPresenter.getActivity().getHandler().post(new Runnable() {
                public void run() {
                    if (mPresenter != null) {
                        Toast toast = Toast.makeText(mPresenter.getActivity(), string, duration);
                        if (centered) {
