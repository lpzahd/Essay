...
public final class CacheService extends IntentService {
    ...
    public static void startCache(final Context context, final boolean checkthumbnails) {
    ...
    public static boolean isCacheReady(final long setId) {
    ...
    public static boolean isPresentInCache(final long setId) {
        ...
        if (CACHE_THREAD.get() == null) {
            ...
            restartThread(CACHE_THREAD, "CacheRefresh", new Runnable() {
                public void run() {
                    ...
                    long ids[] = computeDirtySets(presenter);
                    processQueuedDirty(presenter);
    ...
    public static void markDirtyImmediate(final long id) {
    ...
    public static boolean setHasItems(final ContentResolver cr, final long setId) {
    ...
    public static void populateVideoItemFromCursor(final MediaItem item, final ContentResolver cr, final Cursor cursor,
    ...
    // failed.
    public static long fetchDateTaken(final MediaItem item) {
    ...
    public static ImageList getImageList(final Context context) {
        if (sList != null)
            return sList;
    ...
    private static byte[] queryThumbnail(final MediaWaiter presenter, final long thumbId, final long origId, final boolean isVideo,
        ...
        if (bitmap == null) {
            ...
            bitmap = buildThumbnailForId(presenter, thumbnailCache, thumbId, origId, isVideo, DEFAULT_THUMBNAIL_WIDTH,
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!isInThumbnailerSkipList(thumbnailId)) {
                if (!thumbnailCache.isDataAvailable(thumbnailId, timeModifiedInSec * 1000)) {
                    byte[] retVal = buildThumbnailForId(presenter, thumbnailCache, thumbnailId, id, false, DEFAULT_THUMBNAIL_WIDTH,
    ...
    private static byte[] buildThumbnailForId(final MediaWaiter presenter, final DiskCache thumbnailCache, final long thumbId,
        ...
        try {
            ...
            if (!isVideo) {
                ...
                try {
                    bitmap = UriTexture.createFromUri(presenter.getActivity(), uriString, 1024, 1024, thumbId);
                } catch (IOException e) {
                    return null;
                ...
                new Thread() {
                    public void run() {
                        ...
                        try {
                            MediaStore.Video.Thumbnails.cancelThumbnailRequest(presenter.getActivity().getContentResolver(), origId);
                        } catch (Exception ignored) {
                        }
                    }
                }.start();
                bitmap = MediaStore.Video.Thumbnails.getThumbnail(presenter.getActivity().getContentResolver(), origId,
                        MediaStore.Video.Thumbnails.MICRO_KIND, null);
            }
            if (bitmap == null) {
                return null;
            }
            return writeBitmapToCache(thumbnailCache, thumbId, origId, bitmap, thumbnailWidth, thumbnailHeight,
                    timestamp);
        } catch (InterruptedException e) {
            return null;
        }
    ...
    private static void putLocaleForAlbumCache(final Locale locale) {
    ...
    private static void restartThread(final AtomicReference<Thread> threadRef, final String name, final Runnable action) {
        // Create a new thread.
    ...
    public static void startNewThumbnailThread(final MediaWaiter presenter) {
        restartThread(THUMBNAIL_THREAD, "ThumbnailRefresh", new Runnable() {
            public void run() {
                ...
                CacheService.buildThumbnails(presenter);
            }
        });
    }
    ...
    private static byte[] longArrayToByteArray(final long[] l) {
    ...
    private static void refreshDirtySets(final MediaWaiter presenter) {
        ...
        if (existingData != null && existingData.length > 0) {
            ...
            if (numIds > 0) {
                ...
                populateMediaItemsForSets(presenter.getActivity(), sets, acceleratedSets, true);
                if (!Thread.interrupted()) {
                    int numSets = sets.size();
                    for (int i = 0; i < numSets; ++i) {
                        MediaSet set = sets.get(i);
                        presenter.refreshUIForSet(set);
                    }
                }
            }
        }
    ...
    private static void addNoDupe(ArrayList<Long> array, long value) {
        int size = array.size();
        for (int i = 0; i < size; ++i) {
    ...
    private static void populateMediaItemsForSets(final Context context, final ArrayList<MediaSet> sets,
    ...
    private static void writeSetsToCache(final ArrayList<MediaSet> sets) {
        final ByteArrayOutputStream bos = new ByteArrayOutputStream();
        final int numSets = sets.size();
    ...
    private static void writeItemsToCache(final ArrayList<MediaSet> sets) {
        final int numSets = sets.size();
        for (int i = 0; i < numSets; ++i) {
            if (Thread.interrupted()) {
                return;
    ...
    private static void writeItemsForASet(final MediaSet set) {
        final ByteArrayOutputStream bos = new ByteArrayOutputStream();
        final DataOutputStream dos = new DataOutputStream(new BufferedOutputStream(bos, 256));
        try {
    ...
    private static MediaSet findSet(final long id, final LongSparseArray<MediaSet> acceleratedTable) {
